...
from python.ops.base import array as array_ops
...
class BasicPolicy(Policy):
    ...
    def build(self, input_states, input_actions):
        self._pi = self._build_network(self, input_states)
...
class NormalDistributionPolicy(DistributionPolicy):
    ...
    def build(self, input_states, input_actions):
        mu, sigma = self._build_network(self, input_states)
        self._distribution = tf.contrib.distributions.Normal(mu, sigma, self._validate_args, self._allow_nan_stats)

        super(NormalDistributionPolicy, self).build(input_states, input_actions)
...
class MultiPolicy(Policy):
    ...
    def build(self, input_states, input_actions):
        ...
        for i in xrange(len(self._polices)):
            ...
            ed = st + n
            p.build(input_states, input_actions[:,st:ed])
